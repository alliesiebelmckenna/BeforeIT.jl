...
function Properties(parameters::Dict{String, Any}, initial_conditions)
    ...
    vec_tau_PIT = typeFloat(parameters["vec_tau_PIT"])
    vec_thr_lo_PIT = typeFloat(parameters["vec_thr_lo_PIT"])
    vec_thr_up_PIT = typeFloat(parameters["vec_thr_up_PIT"])
    vec_addition_PIT = typeFloat(parameters["vec_addition_PIT"])
